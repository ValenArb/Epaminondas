...
import { Users, Plus, Package, Clock, Check, Settings, ChevronDown, ChevronRight, Edit2, Trash2, BookOpen, Printer } from 'lucide-react';
...
const MOCK_ANIOS = [
    {
        id: 1, nombre: '1er Año',
        materiales: [
            { id: 101, titulo: 'Módulo Matemática 1', descripcion: '80 páginas' },
            { id: 102, titulo: 'Apunte Historia 1', descripcion: '40 páginas' },
        ]
    },
    {
        id: 2, nombre: '3er Año',
        materiales: [
            { id: 201, titulo: 'Módulo Historia 3er Año', descripcion: '60 páginas' },
            { id: 202, titulo: 'Apunte Biología 3', descripcion: '30 páginas' },
            { id: 203, titulo: 'Módulo Geografía 3', descripcion: '45 páginas' },
        ]
    },
];

const MOCK_FOTOCOPIAS = [
    { id: 1, solicitante: 'Prof. Gómez', material: 'Módulo Historia 3er Año', cantidad: 20, estado: 'pendiente', fecha: '2024-02-01' },
    { id: 2, solicitante: 'Martina (Alumna)', material: 'Apunte Biología 3', cantidad: 1, estado: 'listo', fecha: '2024-02-03' },
    { id: 3, solicitante: 'Colegio San José', material: 'Exámenes de Matemática', cantidad: 30, estado: 'entregado', fecha: '2024-01-28' }
];
...
export default function Fotocopias() {
    ...
    const handleManualNew = () => {
        ...
        setFotocopias([...fotocopias, { id: Date.now(), solicitante, material, cantidad, estado: 'pendiente', fecha: new Date().toISOString().split('T')[0] }]);
    };
    ...
    const handleIngresoMaterial = () => {
        ...
        // Find pending orders for this material, oldest first
        ...
        if (remaining > 0) msg += `\n📄 ${remaining} copias sin pedido pendiente (stock sobrante).`;
                ...
                <div className="space-y-4 flex-1 overflow-y-auto pr-2">
                    {tasks.map(job => (
                        <div key={job.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            ...
                            {job.cantidad > 1 && <p className="text-xs text-blue-600 font-bold mb-2">{job.cantidad} copias</p>}
                                ...
                                {status === 'entregado' && (
                                    <div className="w-full flex justify-between items-center text-gray-400 text-sm py-1 font-medium">
                                        ...
                                        <button onClick={() => setFotocopias(fotocopias.filter(p => p.id !== job.id))}
                                            className="hover:text-red-500 px-2" title="Archivar/Eliminar">
                                            Archivar
                                        </button>
                                    </div>
                                )}
                        ...
                        <div>
                            ...
                            {selectedAnio && (
                                <div className="mt-4 space-y-2">
                                    ...
                                    {anios.find(a => a.id === selectedAnio)?.materiales.map(mat => (
                                        <label key={mat.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedMaterials.includes(mat.titulo) ? 'bg-blue-50 border-blue-300' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                            ...
                            {selectedMaterials.length > 0 && (
                                <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
                                    ...
                                    <ul className="text-sm text-blue-700 space-y-1">
                                        {selectedMaterials.map((t, i) => <li key={i}>• {t} (×{orderQuantity})</li>)}
                                    </ul>
                                </div>
                            )}
    ...
    const CatalogoTab = () => {
        ...
        const handleAddMaterial = (anioId) => {
            ...
            setAnios(anios.map(a => a.id === anioId
                ? { ...a, materiales: [...a.materiales, { id: Date.now(), titulo, descripcion }] }
                : a
            ));
        };
        ...
        return (
            <div>
                <div className="flex justify-between items-center mb-6">
                    <p className="text-gray-500">Configurá los materiales de fotocopias por año/grado. Se usan en el selector de "Nuevo Trabajo".</p>
