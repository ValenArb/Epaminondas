...
const api = {
    ...
    marcarComoLocal: (titulo) => fetch(`${BASE}/stock/marcar-local/`, { method: 'POST', headers: hdr, body: JSON.stringify({ titulo }) }).then(json),
    ingresarStock: (d) => fetch(`${BASE}/stock/`, { method: 'POST', headers: hdr, body: JSON.stringify(d) }).then(json),
